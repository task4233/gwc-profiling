...
## 用語の定義
用語として **プロファイル** 、**プロファイら**、 **プロファイリング**　という言葉が用いられます。これらは様々な使われ方をしますが、本ワークショップでは以下のような意味合いで用います。
...
Go におけるプロファイリングでは、主に pprof が用いられます。pprof は、プロファイルのサンプリング機能と可視化機能の2つの機能を持ち合わせています。
...
👉 **「この関数の中身がどれだけ重いか」を見る指標**

##### cum（cumulative / 累積コスト）

**意味**: その関数が呼ばれている間に消費された合計のリソース量

**含まれるもの**:
- flat（自分自身）
- その関数から呼び出されたすべての子関数のコスト

**含まれないもの**:
- 呼び出し元（親関数）のコスト

👉 **「この関数を起点とした処理全体がどれだけ重いか」を見る指標**
...
cumが大きくてもflatが小さければ、その関数自体は軽い。
重いのは呼び出し先なので、call graphをたどって真のボトルネックを探す必要がある。
...
| パターン | flat | cum | 解釈 | 対処 |
|---------|------|-----|------|------|
| **パターン1** | 大 | 大 | 関数自体が重い | その関数を最適化 |
| **パターン2** | 小 | 大 | 呼び出し先が重い | call graphをたどって重い子関数を探す |
| **パターン3** | 大 | 大（flat≈cum） | 末端の重い関数 | 最優先で最適化すべき |

**重要**: `flat`が大きい関数はその関数自体の最適化が効果的。`cum`が大きく`flat`が小さい関数は、呼び出し先の最適化を検討すべき。
...
**分析のコツ**:
1. `flat%`が大きい → その関数自体を最適化
2. `cum%`が大きく`flat%`が小さい → 呼び出し先を調査
3. `sum%`が80%に達するまでの関数群 → 主要なボトルネック
...
**判断のポイント**:
- `flat%`が大きい → その関数自体を最適化
- `cum%`が大きく`flat%`が小さい → 呼び出し先を調査
- `sum%`が80%までの関数群 → パレートの法則（20%の関数が80%のリソースを消費）
...
特定の関数のソースコードと、各行のコスト（CPU時間やメモリ）を表示します。
